            ...
            <tbody>
                @foreach ($users as $user)
                    <tr>
                        ...
                        <th scope="row">
                            <a type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#editUserModal{{ $user->id }}">
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content p-1">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editUserModalTitle{{ $user->id }}">Edit News ({{ $user->user_name }})</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <form action="{{ route('edit-userData', ['id' => $user->id]) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <label class="input-group-text" for="inputGroupSelect{{ $user->id }}">Role</label>
                                                    </div>
                                                    <select class="custom-select" name="role" id="inputGroupSelect{{ $user->id }}" aria-label="Example select with button addon">
