                ...
                <tbody>
                    @foreach ($games as $game)
                        <tr>
                            ...
                            <th scope="row">
                                @if ($game->image)
                                    <img src="{{ asset('images/Games/games/' . $game->image) }}" alt="Image"
                                        class="w-75">
                                @endif
                            </th>
                                ...
                                @if (!$game['getGameAnswers']->isEmpty())
                                    <div class="row justify-content-center">
                                        @foreach ($game['getGameAnswers'] as $item)
                                            <div class="col-md-2 gameAnswers m-2 {{ $item->id == $game['getGameRightAnswer']['answer_id'] ? 'rightAnswer' : '' }}"
                                                data-answerId="{{ $item->id }}" data-gameId="{{ $game->id }}">
                                                {{ $item->answer != null ? $item->answer : '' }}

                                                @if ($item->image != null)
                                                    <img src="{{ asset('images/Games/answers/' . $item->image) }}"
                                                        alt="Image" class="w-50" style="height: 70px;">
                                                @endif
                                            </div>
                                            <a type="button" class="btn btn-warning btn-sm" data-toggle="modal"
                                                data-target="#editGameAnswerModal{{ $item->id }}"
                                                style="display: contents;">
                                                <i class="fas fa-edit"></i>
                                            </a>

                                            <!-- Edit News Modal -->
                                            <div class="modal fade" id="editGameAnswerModal{{ $item->id }}"
                                                tabindex="-1" role="dialog"
                                                aria-labelledby="editGameAnswerModalTitle{{ $item->id }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content p-1">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title text-dark"
                                                                id="editGameAnswerModalTitle{{ $item->id }}">Edit Game
                                                                Answer ({{ $item->id }})</h5>
                                                        ...
                                                        <div class="modal-body position-relative">
                                                            <form
                                                                action="{{ route('edit-changed-game', ['id' => $item->id]) }}"
                                                                        ...
                                                                        <div class="input-group mb-3">
                                                                            ...
                                                                            <div class="custom-file">
                                                                                ...
                                                                                <input type="hidden"
                                                                                    ...
                                                                                    name="oldImage"
                                                                                    value="{{ $item->image != null ? $item->image : '' }}">
