...
{{--                    <th scope="col">Image</th>--}}
                ...
                <tbody>
                    <tr>
                        <th scope="row">{{ $features[0]->id }}</th>
{{--                        <th scope="row">--}}
{{--                            <img src="{{ asset('images/' . $about->image) }}" alt="{{ $about->title }}" class="img-fluid w-50" />--}}
{{--                        </th>--}}
                        <th scope="row">{{ $features[0]->title }}</th>
                        <th scope="row">
                            {!! \App\Helpers\TextLimit::limit($features[0]->description) !!}
                        </th>
                        <th scope="row">
                            <a type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#editFeaturesModal{{ $features[0]->id }}">
                ...
                <!-- Edit About Modal -->
                <div class="modal fade" id="editFeaturesModal{{ $features[0]->id }}" tabindex="-1" role="dialog" aria-labelledby="editFeaturesModalTitle{{ $features[0]->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content p-1">
                            <div class="modal-header">
                                <h5 class="modal-title" id="editFeaturesModalTitle{{ $features[0]->id }}">Edit Features</h5>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="tab-content" id="myTabContent">
                                    @foreach($features as $item)
                                        <div class="tab-pane fade show {{ $item->lang == "en" ? "active" : "" }}" id="{{ $item->lang }}" role="tabpanel" aria-labelledby="{{ $item->lang }}-tab">
                                            <form action="{{ route('edit-features', ['lang' => $item->lang]) }}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                <div class="row">
